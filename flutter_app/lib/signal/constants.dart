...
// --- 아티팩트 규모 기준 ---
//
// 잡음 배수만으로는 임계가 정해지지 않는다. 캘리브 창이 유난히 조용한
// 세션에서는 σ=8 → 임계 65 가 되어 아티팩트(약 900) 훨씬 아래로 내려가고,
// M-wave·EMG·아티팩트 꼬리가 전부 임계를 넘어 한 자극이 여러 번 검출된다
// (실측 raw_20260721_102627.csv: epb 38.2, 25ms 미만 간격 8735건).
//
// 반대로 도입부가 시끄러운 세션에서는 σ=311 → σ×8 = 2491 이 되어 |신호−DC|
// 최대치 2202 를 넘고 602초짜리 세션이 통째로 검출 0이 된다
// (raw_20260730_205037_C_complete.csv).
//
// 즉 잡음 배수는 위아래 어느 쪽으로도 신뢰할 수 없다. 임계는 **아티팩트
// 진폭 규모**에 묶는다. 26세션 비교(불응기 26ms):
//
